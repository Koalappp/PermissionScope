...
@objc public class PermissionScope: UIViewController, UIGestureRecognizerDelegate {
	...
	*/
	func showDeniedAlert(permission: PermissionType) {
		...
		let alert = PSTAlertController(title: "Permission for \(permission.prettyDescription) was denied.".localized,
			message: "Please enable access to \(permission.prettyDescription) in the Settings app".localized,
			preferredStyle: .Alert)
		alert.addAction(PSTAlertAction(title: "OK".localized,
			style: .Cancel,
			handler: nil))
		...
		dispatch_async(dispatch_get_main_queue()) {
			guard let vc = self.viewControllerForAlerts else { return }
			alert.showWithSender(self, controller: vc, animated: true, completion: nil)
		}
	}
	...
	*/
	func showDisabledAlert(permission: PermissionType) {
		...
		let alert = PSTAlertController(title: "\(permission.prettyDescription) is currently disabled.".localized,
